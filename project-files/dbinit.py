...
class Database(object):
    ...
    def retrieve_user(self, userName=None, userID=None):
        ...
        if (userName is not None) and (userID is not None):
            ...
            c.execute('''SELECT * FROM User WHERE ID=?''', (userID, ))  # User needs to be a tuple
            res = c.fetchone()
        else:
            raise TypeError
        self._disconnect(conn, c)
    ...
    def retrieve_login(self, day=datetime.today(), time_start=None, time_stop=None):
        ...
        if (time_start is None) or (time_stop is None):  # if timestamps are inserted, they take precedence
            time_start = datetime.combine(day, datetime.min.time())  # starts the query at 00:00:00
            time_stop = datetime.combine(day, datetime.min.time(
            )) + timedelta(days=1) - timedelta(seconds=1)  # ends the query at 23:59:59

        conn, c = self._connect()
        c.execute('''SELECT P.UserID, P.Access, U.UserType FROM Platform as P, User as U where U.ID=P.UserID AND Access between ? and ? ORDER BY P.Access ASC''', (time_start, time_stop,))
        res = c.fetchall()
        self._disconnect(conn, c)
        return res
        ...
        if last:
            ...
            if (time_start is None) or (time_stop is None):
                ...
                if patient is None:
                    ...
                    sql = '''SELECT * FROM ''' + parameter + \
                        ''' WHERE PatientID=? and (Recorded BETWEEN ? and ?) ORDER BY Recorded'''
    ...
    def retrieve_stress(self, patient, time_start, time_stop):
        conn, c = self._connect()

        sql = '''SELECT S.PatientID, S.StressLevel, V.VisitDate FROM PercievedStress as S, Visit as V WHERE S.PatientID=? AND S.VisitID=V.ID AND(V.VisitDate BETWEEN ? and ?) ORDER BY V.VisitDate'''
        c.execute(sql, (patient, time_start, time_stop))
    ...
    def edit_visit(self, VisitID, PatientID, MedicianID, VisitDate, PSG, Complete, Anamnesis, Diagnosis, Notes, Prescription, Full=True):
        conn, c = self._connect()
        if Full:
    ...
    def insert_activity(self, userID, hours):
        ...
        VALUES(?,?,?,?)''', (userID, datetime.today(), hours, alert))
        self._disconnect(conn, c)
        ...
        if visitID is None:
            if all:
                c.execute('''SELECT
                        ...
                        V.ID, V.VisitDate, V.PSG, V.Complete
                        FROM
